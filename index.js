...
// --- FONCTION DE MISE À JOUR DE L'INTERFACE DEPUIS INDEXEDDB ---
async function chargerAlarmesDepuisIndexedDB() {
    try {
        const tableau_final = await db.alarmesActives.toArray();
        
        // 1. Compteur d'alarmes actives
        const compteur = document.getElementById('compteur-actifs');
        if (compteur) compteur.textContent = tableau_final.length;

        // 2. Calculs pour la barre de répartition
        ...
        const elDefaut = document.getElementById('txt-count-defaut');
        const elAvert = document.getElementById('txt-count-avertissement');
        const elIndic = document.getElementById('txt-count-indication');

        if (elDefaut) elDefaut.textContent = nbre_defaut;
        if (elAvert) elAvert.textContent = nbre_avertissement;
        if (elIndic) elIndic.textContent = nbre_indication;

        if (total > 0) {
            document.getElementById('barre-defaut').style.width = `${(nbre_defaut / total) * 100}%`;
            document.getElementById('barre-indication').style.width = `${(nbre_indication / total) * 100}%`;
            document.getElementById('barre-avertissement').style.width = `${(nbre_avertissement / total) * 100}%`;
        } else {
        ...
        // 3. Injection dans le tableau HTML
        const tbody = document.getElementById('tbody-messages-actifs');
        if (!tbody) return;

        if (tableau_final.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:10px;">Aucun message actif en mémoire. Veuillez charger un fichier XML.</td></tr>`;
        } else {
            let htmlGenerer = "";
            tableau_final.forEach(a => {
                let classeBadge = "badge-warning";
                let libelleType = "Avertissement";
        ...
        console.error("Erreur de chargement depuis IndexedDB :", err);
    }
}

// --- INITIALISATION AU CHARGEMENT & IMPORT XML ---
document.addEventListener('DOMContentLoaded', () => {
    // 1. Charger immédiatement les dernières alarmes en mémoire Dexie
    chargerAlarmesDepuisIndexedDB();

    // 2. Écouteurs pour l'import d'un nouveau fichier XML
        ...
        inputExcel.addEventListener('change', () => {
            ...
            lecteur.onload = async function (evenement) {
                try {
                    ...
                    const tableauHistorique = Array.from(nœudsHistorique).map(alarm => ({
                    ...
                    // Extraction Alarmes Actives
                    const alarmes_actives = xmlDoc.querySelectorAll('Apartment[Type="Active Alarms"] Alarm');
                    const tableau_final = Array.from(alarmes_actives).map(alarm => ({
                    ...
                    // Réactualiser l'affichage avec les données fraîchement enregistrées
                    await chargerAlarmesDepuisIndexedDB();
